...
//     playPromise
//       .then(() => {
//         console.log("Âm nhạc đã phát tự động.");
//       })
//       .catch(() => {
...
const characters = "HAPPYBIRTHDAYCHUCMUNGSINHNHATTRANTHICAMHONG";
const fontSize = 20;
...
function getTextPixels(text) {
  ...
  while (offCtx.measureText(text).width > W * 0.9) {
    fontSizeOverlay -= 2;
    offCtx.font = `bold ${fontSizeOverlay}px "Pacifico"`;
  }
...
currentDisplayTime = BASE_DISPLAY_TIME + (texts[0].length > 5 ? 3000 : 0);
initDots(texts[currentTextIndex]);
